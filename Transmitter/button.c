...
void Button_UpdateCAN(PushButton *btn, CANBUS *can)
{
    ...
    if (btn->last_state == GPIO_PIN_SET && state == GPIO_PIN_RESET) /* check if state is changed from initial */
    {
        uint32_t now = HAL_GetTick();

        if (btn->last_press_time == 0 || (now - btn->last_press_time) >= BUTTON_DEBOUNCE_MS) /* check for debounce timer */
        {
            btn->last_press_time = now;

            if (btn->counter) {
                (*btn->counter)++; /* increment counter and send over CAN with the message */
